...
export default function DashboardPage() {
  ...
  // NOVO: Estado para guardar o status real do mercado (Ibovespa)
  const [marketStatus, setMarketStatus] = useState<any>(null);
  
  const supabase = createClient();

  useEffect(() => {
    // Rotina 1: Busca os Trades Ativos
    ...
    fetchActiveTrades();
    fetchMarketStatus(); // Chama a rotina dinamicamente ao carregar a página
  }, [supabase]);
    ...
    <div className="flex h-screen w-full bg-background-dark overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto overflow-x-hidden relative">
        ...
        <div className="p-4 md:p-8 space-y-6 md:space-y-8 max-w-[1600px] mx-auto w-full pb-20">
          ...
          {/* TRADUZIDO: Visão Geral do Mercado */}
          <motion.div
          ...
          {/* TRADUZIDO: Monitor de Operações */}
          <motion.section
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-8">
            ...
            {/* Coluna Lateral: Sentimento e Status Widget */}
            <motion.div 
              className="space-y-8"
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              ...
              {/* DINÂMICO E TRADUZIDO: Market Status Widget */}
